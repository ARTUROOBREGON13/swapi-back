def longest_pokemon_sequence():
    pokemons = "audino bagon baltoy banette bidoof braviary bronzor carracosta charmeleon "\
        "cresselia croagunk darmanitan deino emboar emolga exeggcute gabite girafarig gulpin "\
        " haxorus heatmor heatran ivysaur jellicent jumpluff kangaskhan kricketune "\
    ...
    def pokemon_starts_with(last_character, poke_list):
        ...
        Args:
            last_character (str): The last character to compare with the first of every item in the poke_list
            poke_list (list): The remaining unique elements in the array that can be compared against the last character

        Returns:
            int/boolean: int if it finds an item in "poke_list" that starts with the "last character" or boolean, 
            False, when it doesn't find a match.
        """
    ...
    def generate_list(first_name, poke_list):
        """Function to create the temporal list that is started by the first_name provided

        Args:
            first_name (str): First element on the list
            poke_list (list): A copy of the original array of names which will be modified with every iteration.

        Returns:
            list: The resulting list of pokemon names in the order that are inserted to the list.
        """
